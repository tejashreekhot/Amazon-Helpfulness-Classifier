...
def getmyxls():
    ...
    for row in sheetName.rows:
        ...
        print r
        fulllist.append(r)
    listwewant=[(unicode('H'),((removepunctuation(removebrtag(x[0])).lower()),removepunctuation(removebrtag(x[1])).lower())) if x[-1]>=0.65 else (unicode('NH'),(removepunctuation(removebrtag(x[0])).lower(),removepunctuation(removebrtag(x[1])).lower()))for x in fulllist]
    print "hey"
    print(listwewant)
    return listwewant
#####################################
...
def getLabelsAndVectors(dataTuples):
    ...
    for dataPoint in dataTuples:
        ids.append(c)
        c+=1
        label, vector=dataPoint[0], (dataPoint[1][0].split(),dataPoint[1][1].split())
